...
```bash
git clone https://github.com/Adihang/HanPlanet-CLI.git
cd HanPlanet-CLI
...
> echo "/path/to/HanPlanet-CLI
> /path/to/HanPlanet-CLI/src" \
>   > ~/.local/pipx/venvs/hanplanet-cli/lib/python3.13/site-packages/_hanplanet_cli.pth
> ```
...
**파일:** `src/openharness/ui/backend_host.py`, `src/openharness/config/settings.py`

- Hanplanet이 기본 카탈로그(`default_provider_profiles`)에 포함 — fresh install에서도 목록에 표시
...
시스템 프롬프트에서 자기 소개 이름을 `OpenHarness` → `HanPlanet CLI` 로 변경.

---

### 배너 변경

**파일:** `frontend/terminal/src/components/WelcomeBanner.tsx`

시작 시 표시되는 웰컴 배너를 HANPLANET 6행 ASCII 아트로 교체.

- 각 줄마다 다른 hex 색상 적용 (ansi256 18→33 다크→라이트 블루 그라데이션)
- "HanPlanet CLI" 서브타이틀 (`#00afff`)
- `www.hanplanet.com` + 버전 표기
- `chalk.ansi256()` 대신 Ink 네이티브 `color` prop 사용 → 터미널 리사이즈 시 배너 깨짐 방지

```
hex 매핑:
  줄 1: #000087  (ansi256 18)
  줄 2: #0000af  (ansi256 19)
  줄 3: #0000d7  (ansi256 20)
  줄 4: #0000ff  (ansi256 21)
  줄 5: #005fff  (ansi256 27)
  줄 6: #0087ff  (ansi256 33)
  subtitle: #00afff  (ansi256 39)
```

---

### 인증 미설정 시 Provider 선택창 자동 실행

**파일:** `src/openharness/ui/runtime.py`, `src/openharness/ui/backend_host.py`

API 키 없이 실행하면 에러로 종료되는 대신, 앱이 정상 기동하고 즉시 `/provider` 선택창을 자동으로 엽니다.

- **기존:** `Error: No API key configured.` → 프로그램 종료
- **변경:** 더미 클라이언트로 런타임 시작 → `ready` emit 후 `auth_status == "missing"` 감지 → `/provider` 자동 실행

---

### npm 의존성 설치 진행 표시

**파일:** `src/openharness/ui/react_launcher.py`

최초 실행 시 `node_modules`가 없으면 `npm install`을 실행하는데, 기존에는 아무 출력 없이 멈춘 것처럼 보였음.

- `rich.progress` 스피너로 설치 진행 상태 표시
- 설치 완료 후 `✅ 설치 완료!` 메시지 출력

---

### `/update` — 소스 업데이트 커맨드

**파일:** `src/openharness/commands/registry.py`

앱 실행 중 `/update` 입력으로 최신 소스를 받아 업데이트.

| 커맨드 | 동작 |
|--------|------|
| `/update` | `git pull origin main` 실행 → 변경 있으면 `pip install -e .` 자동 재설치 |
| `/update force` | 버전 무관하게 강제 `pip install -e .` (pipx upgrade 버전 비교 우회) |

- editable 설치(`pipx install -e .`) 환경 전용
- 업데이트 후 ctrl+c → 재실행으로 변경사항 적용
...
**파일:** `src/openharness/engine/query.py`, `src/openharness/api/openai_client.py`, `src/openharness/api/codex_client.py`, `src/openharness/tools/file_write_tool.py`

로컬 모델이 `write_file` tool-call을 호출하면서 인자를 비워 보내거나 깨진 JSON arguments를 보내 파일 생성에 실패하는 문제를 방지.

- `write_file` 필수 인자(`path`, `content`) 누락 시 짧고 명확한 오류 메시지 표시
- tool input 오류 발생 직후 자동 복구 턴 주입 (`Retry the failed tool call now with valid JSON arguments`)
- OpenAI-compatible/Codex 파서에서 깨진 tool arguments를 `{}`로 숨기지 않고 원문 일부를 보존해 원인 표시
- `write_file` schema 설명에 필수 JSON 예시 추가 (`{"path":"index.html","content":"..."}`)
...
빌드 결과물은 단계마다 이름이 다르다.

| 위치 | macOS | Windows |
|------|-------|---------|
| GitHub Actions artifact / Release asset | `HanPlanet-CLI-macos-arm64.zip` | `HanPlanet-CLI-windows-x64.zip` |
...
이 규칙에 맞춰 다음 경로들도 같이 수정되어 있어야 한다.

- Hanplanet 웹의 `/handrive/cli` 다운로드 버튼
- standalone 번들의 `/update` 명령어
...
| 파일 | 보호해야 할 커스텀 내용 |
|------|------------------------|
| `src/openharness/ui/backend_host.py` | `_hanplanet_oauth_flow`, `_hanplanet_save_and_select`, `_hanplanet_refresh_token`, `_maybe_refresh_hanplanet_token`, `_fetch_hanplanet_models`, `command == "model-for-hanplanet"`, `command == "provider"` 내 Hanplanet 분기, `command == "model"` 내 Hanplanet 분기, `_VISIBLE_PROFILES` |
| `frontend/terminal/src/components/WelcomeBanner.tsx` | HANPLANET ASCII 아트 배너, hex 그라데이션 색상, HanPlanet CLI 서브타이틀 |
| `src/openharness/prompts/system_prompt.py` | `_BASE_SYSTEM_PROMPT` 첫 줄 (`You are HanPlanet CLI`) |
| `pyproject.toml` | `[project.scripts]` 의 `hanplanet` entry point, `name = "HanPlanet-CLI"` |
