...
def create_default_command_registry(
    ...
    async def _skills_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not args or sub == "list":
            ...
            lines.append("(* = preloaded into system prompt)")
            for skill in skills:
                ...
                lines.append(f"- {command_name}{display}{source}{path}{slash}{marker}: {skill.description}")
            if preloaded:
            ...
            try:
                ...
                if action == "uninstall" and plugin_names:
                    ...
                    try:
                        ...
                        return CommandResult(message=f"Invalid plugin name '{name}'")
                    if removed:
    ...
    registry.register(
        SlashCommand(
            "language",
            "Show or set AI response language",
            _language_handler,
            remote_invocable=False,
            remote_admin_opt_in=True,
        )
